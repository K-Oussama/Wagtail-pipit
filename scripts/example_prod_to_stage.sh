...
readonly STAGE_HOST=deploy@stage.SimpleBlog.com
readonly PROD_HOST=deploy@SimpleBlog.com
...
ssh $PROD_HOST "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres simpleblog -x -O"
...
ssh $STAGE_HOST "sudo -u postgres pg_restore --clean -h localhost -d simpleblog -U postgres '/tmp/db-dump.sql'"
ssh $STAGE_HOST "rm /tmp/db-dump.sql"

rm /tmp/db-dump.sql

echo "Updating database..."
manage_prefix="source /mnt/persist/www/simpleblog/shared/venv/bin/activate && cd /mnt/persist/www/simpleblog/current/src &&"

ssh $STAGE_HOST "$manage_prefix python manage.py change_site_domain --site_id=1 --new_site_domain='stage.SimpleBlog.com'"

echo "Syncing s3 buckets..."
aws --profile simpleblog_devops s3 sync s3://s3.stage.SimpleBlog.com s3://s3.SimpleBlog.com --acl public-read
