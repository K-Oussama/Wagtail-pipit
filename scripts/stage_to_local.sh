...
readonly LOCAL_DOMAIN=SimpleBlog.com.test:8081
readonly SSH_HOST=deploy@stage.SimpleBlog.com
readonly REMOTE_MEDIA_PATH=/mnt/persist/www/simpleblog/shared/media

scripts_dir="$(cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd)"
docker_dir=${scripts_dir}/../docker

echo "Creating database dump from stage..."
ssh $SSH_HOST "export PGUSER=postgres && pg_dump simpleblog --no-owner > /tmp/db-dump.sql"
